...
一気に全部やらない。進行の正本は [`PROGRESS.md`](PROGRESS.md)。計画の 0–5、7、10–12 は完了、6 はキャンセル。日常モデルは Qwen3.5-9B Q4。追加実験は新しいフェーズを切ってから。並行して、行列積から超小型 LM までを手で書く基礎路 [`primer/`](primer/)（GPU 不要。教材は計画のみ）。
...
先に読む教材: [`notes/learn-llm-basics.html`](notes/learn-llm-basics.html)（これまで動かしてきたものの中身。13 の前）、[`notes/learn-sft-lora.html`](notes/learn-sft-lora.html)（SFT と LoRA。14 の前）。式の中身（行列積・勾配）で止まったら並行して [`primer/README.md`](primer/README.md)。
...
| [`primer/`](primer/) | 基礎サブプロジェクト。理論 HTML + 原始スクリプト（標準ライブラリ）。計画は [`primer/README.md`](primer/README.md)。13–17 とは別トラック |
